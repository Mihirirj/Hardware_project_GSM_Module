...
adc_init();
	...
	while(1){
char array[6];
		adc_value = adc_read(4);	/* PH value*/
		PH = 100-(adc_value*100.00)/1023.00;
		dtostrf(PH,3,2,array);
		strcat(array,"%   ");
		LCD_String_xy(0,1,array);
	...
	if (PH>28.0 && PH<32)
	{
			...
			GSM_Send_Msg("0775686856","PH Value is not good");
			LCD_Clear();
			LCD_String_xy(1,0," Massage Sent");
